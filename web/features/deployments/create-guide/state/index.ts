...
import type { DeployRequest, EnvVarInput } from '@dify/contracts/enterprise/types.gen'
...
} from '@/features/deployments/components/env-var-bindings-utils'
import {
...
export const effectiveSelectedAppAtom = atom((get) => {
  ...
  if (sourceAppsQuery.isPlaceholderData) return undefined
...
const deploymentOptionsReadyAtom = atom((get) => {
  ...
  return sourceReady(get) && get(precheckReleaseReadyAtom) && deploymentOptionsQuery.isSuccess
})
...
const deploymentOptionsContentCheckedAtom = atom((get) => {
  ...
  const isCheckingReleaseContent =
    precheckReleaseQuery.isLoading ||
    (precheckReleaseQuery.isFetching && !precheckReleaseQuery.data)
...
export const setSourceSearchTextAtom = atom(null, (get, set, value: string) => {
  if (get(sourceSearchTextAtom) === value) return
...
export const continueFromSourceAtom = atom(
  null,
  (
    get,
    set,
    ...
    if (!get(sourceCanGoNextAtom)) return

    const method = get(effectiveMethodAtom)
    const effectiveSelectedApp = get(effectiveSelectedAppAtom)
    if (method === 'bindApp' && effectiveSelectedApp) set(selectSourceAppAtom, effectiveSelectedApp)
...
export const deployableEnvironmentsAtom = atom((get) => {
  ...
  return sourceReady(get) ? (deployableEnvironmentsQuery.data?.environments ?? []) : []
})

...
} from './overview-tab/access-status-section'
  ...
  return (
    <section className="flex min-w-0 flex-col gap-3">
      <div className="flex min-w-0 items-baseline justify-between gap-3">
        <h3 className="system-sm-semibold text-text-primary">{t('overview.latestReleaseTitle')}</h3>
        <Link
...
function OverviewLoadingSkeleton({ appInstanceId }: { appInstanceId: string }) {
  return (
    <OverviewLayout>
...
export function OverviewTab({ appInstanceId }: { appInstanceId: string }) {
  const { t } = useTranslation('deployments')
  const overviewQuery = useAtomValue(deploymentDetailOverviewQueryAtom)
  const overview = overviewQuery.data

  if (overviewQuery.isLoading) return <OverviewLoadingSkeleton appInstanceId={appInstanceId} />

...
function AgentToolItem({
  ...
  return <AgentCliToolItem tool={tool} onDelete={handleDeleteCliTool} onEdit={handleEditCliTool} />
}
...
function AddToolMenu({
  ...
  const toAgentToolDefaultValue = useCallback(
    (tool: ToolDefaultValue): AgentProviderToolDefaultValue => ({
      ...
      credentialRequired: getProviderCredentialRequired(
        providerById.get(tool.provider_id) ??
          providerById.get(tool.provider_name) ??
          (tool.plugin_id ? providerById.get(tool.plugin_id) : undefined),
      ),
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger
        render={<ConfigureSectionAddButton ariaLabel={t('agentDetail.configure.tools.add')} />}
      />
      <PopoverContent
...
export function AgentTools() {
  ...
  useEffect(() => {
    ...
    const nextTools = tools.map((tool, index) => {
      ...
      if (tool.kind !== 'provider' || displayTool?.kind !== 'provider') return tool
    ...
    if (shouldSyncCredentials) setTools(nextTools)
  ...
  const settingTargetCollection = settingTarget
    ? (providerById.get(settingTarget.tool.id) ?? providerById.get(settingTarget.tool.name))
    : undefined

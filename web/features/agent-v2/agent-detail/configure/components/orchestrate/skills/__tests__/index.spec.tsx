...
describe('AgentSkills', () => {
  ...
  it('should fetch drive files and open the skill detail dialog when the skill row is clicked', async () => {
    ...
    expect(
      within(dialog).getByText('Extracts tender requirements and scoring criteria.'),
    ).toBeInTheDocument()
  ...
  it('should preview selected package file content in the skill detail dialog', async () => {
    ...
    expect(
      await within(dialog).findByText('Preview content for tender-analyzer/references/guide.md'),
    ).toBeInTheDocument()
  ...
  it('should not request preview for package files without a drive entry', async () => {
    ...
    expect(
      within(dialog).getByText('agentV2.agentDetail.configure.files.preview.empty'),
    ).toBeInTheDocument()
  })
  ...
  it('should deduplicate package files by path in the skill detail dialog', async () => {
    ...
    expect(
      within(dialog).getByText('agentV2.agentDetail.configure.skills.detail.fileCount:{"count":2}'),
    ).toBeInTheDocument()
  })
  ...
  it('should route Agent App skill delete through agent and slug identifiers', async () => {
    const deleteSkill = vi
      .fn()
      .mockResolvedValue({ result: 'success', removed_keys: ['tender-analyzer/SKILL.md'] })
    mocks.deleteSkillMutationOptions.mockReturnValue({
  ...
  it('should upload a skill through the drive-backed endpoint', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'agentV2.agentDetail.configure.skills.add' }),
    )
    ...
    await user.click(
      screen.getByRole('button', { name: 'agentV2.agentDetail.configure.skills.upload.action' }),
    )
    ...
    expect(vi.mocked(toast.success)).toHaveBeenCalledWith(
      'agentV2.agentDetail.configure.skills.upload.success',
    )
  })
  ...
  it('should not persist skills_files when the draft is serialized', async () => {
    ...
    await waitFor(() => {
      const serializedConfig = JSON.parse(
        screen.getByTestId('config-snapshot-probe').textContent ?? '{}',
      )

...
import {
  ...
  DeploymentDetailTop,
} from '@/features/deployments/detail/deployment-sidebar'
...
const MainNav = ({ className }: MainNavProps) => {
  ...
  const { langGeniusVersionInfo, isCurrentWorkspaceDatasetOperator, isCurrentWorkspaceEditor } =
    useAppContext()
  ...
  const showAgentDetailNavigation =
    agentV2Enabled && !isCurrentWorkspaceDatasetOperator && isAgentDetailPathname(pathname)
  const showDeploymentDetailNavigation =
    canUseAppDeploy && !isCurrentWorkspaceDatasetOperator && isDeploymentDetailPathname(pathname)
  const showSnippetDetailBottomNavigation = isSnippetDetailPathname(pathname)
  const showDetailNavigation =
  ...
  const isDetailNavigationHoverPreviewOpen =
    isCollapsedDetailNavigation && detailNavigationHoverPreviewOpen
  ...
  const openDetailNavigationHoverPreview = useCallback(() => {
    if (!isCollapsedDetailNavigation) return
  ...
  useHotkey(
    'Mod+B',
    (e) => {
      if (!showDetailNavigation) return

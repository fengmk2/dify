...
const { mockIsAgentV2Enabled, mockSwitchWorkspace, mockToastSuccess, hotkeyRegistrations } =
  vi.hoisted(() => ({
...
vi.mock('@/app/components/app-sidebar/app-detail-section', () => ({
  default: ({ expand }: { expand: boolean }) => (
    <div data-testid="app-detail-section" data-expand={expand} />
  ),
}))
...
vi.mock('@/app/components/app-sidebar/app-detail-top', () => ({
  default: ({ expand, onToggle }: { expand: boolean; onToggle: () => void }) => (
    <div data-testid="app-detail-top" data-expand={expand}>
      <button type="button" data-testid="app-detail-toggle" onClick={onToggle}>
        Toggle
      </button>
    </div>
  ),
...
vi.mock('@/app/components/app-sidebar/dataset-detail-section', () => ({
  default: ({ expand }: { expand: boolean }) => (
    <div data-testid="dataset-detail-section" data-expand={expand} />
  ),
}))
...
vi.mock('@/app/components/app-sidebar/dataset-detail-top', () => ({
  default: ({ expand, onToggle }: { expand: boolean; onToggle: () => void }) => (
    <div data-testid="dataset-detail-top" data-expand={expand}>
      <button type="button" data-testid="dataset-detail-toggle" onClick={onToggle}>
        Toggle
      </button>
    </div>
  ),
...
vi.mock('@/features/agent-v2/agent-detail/navigation', () => ({
  AgentDetailSection: ({ expand }: { expand: boolean }) => (
    <div data-testid="agent-detail-section" data-expand={expand} />
  ),
  AgentDetailTop: ({ expand, onToggle }: { expand: boolean; onToggle: () => void }) => (
    <div data-testid="agent-detail-top" data-expand={expand}>
      <button type="button" data-testid="agent-detail-toggle" onClick={onToggle}>
        Toggle
      </button>
    </div>
  ),
...
vi.mock('@/features/deployments/detail/deployment-sidebar', () => ({
  DeploymentDetailSection: ({ expand }: { expand: boolean }) => (
    <div data-testid="deployment-detail-section" data-expand={expand} />
  ),
  DeploymentDetailTop: ({ expand, onToggle }: { expand: boolean; onToggle: () => void }) => (
    <div data-testid="deployment-detail-top" data-expand={expand}>
      <button type="button" data-testid="deployment-detail-toggle" onClick={onToggle}>
        Toggle
      </button>
    </div>
  ),
...
const renderMainNav = (
  ...
  queryClient.setQueryData(
    ...
    currentAppContext.currentWorkspace,
  )
...
describe('MainNav', () => {
  beforeEach(() => {
    ...
    ;(useAppContextSelector as Mock).mockImplementation(
      (selector: (state: AppContextValue) => unknown) =>
        selector((useAppContext as Mock)() as AppContextValue),
    )
  ...
  it('renders primary navigation with the planned routes', () => {
    ...
    expect(screen.getByRole('link', { name: /common.menus.roster/ })).toHaveAttribute(
      'href',
      '/roster',
    )

...
const SideBar = () => {
  ...
  return (
    <div
      className={cn(
        'flex h-full w-fit shrink-0 cursor-pointer flex-col px-3 pt-6 sm:w-[240px]',
        isFold && 'sm:w-[56px]',
      )}
    >
      ...
      {installedApps.length > 0 && (
        <div className="mt-5 flex min-h-0 flex-1 flex-col">
          {!isMobile && !isFold && (
            <p
              id={webAppsLabelId}
              className="mb-1.5 pl-2 system-xs-medium-uppercase break-all text-text-tertiary uppercase mobile:px-0"
            >

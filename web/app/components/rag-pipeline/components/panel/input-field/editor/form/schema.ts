...
const TransferMethod = z.enum(['all', 'local_file', 'remote_url'])

const SupportedFileTypes = z.enum(['image', 'document', 'video', 'audio', 'custom'])
...
export const createInputFieldSchema = (
  ...
  const commonSchema = z.object({
    ...
    variable: z
      .string()
      .nonempty({
        message: t('varKeyError.canNoBeEmpty', {
          ns: 'appDebug',
          key: t('variableConfig.varName', { ns: 'appDebug' }),
        }),
      })
      .max(MAX_VAR_KEY_LENGTH, {
        message: t('varKeyError.tooLong', {
          ns: 'appDebug',
          key: t('variableConfig.varName', { ns: 'appDebug' }),
        }),
      })
      .regex(/^(?!\d)\w+/, {
        message: t('varKeyError.notStartWithNumber', {
          ns: 'appDebug',
          key: t('variableConfig.varName', { ns: 'appDebug' }),
        }),
      })

...
- `tool_icons`(object[string]) ツールアイコン - `tool_name` (string) - `icon` (object|string) - (object) アイコンオブジェクト - `background` (string) 背景色（16 進数形式）- `content`(string) 絵文字 - (string) アイコンの URL

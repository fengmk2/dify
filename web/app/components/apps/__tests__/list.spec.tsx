...
describe('List', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render sort filter before search and hide the snippets link', () => {
      ...
      expect(
        searchInput.compareDocumentPosition(createButton) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy()
      expect(
        screen.queryByRole('link', { name: 'app.studio.viewSnippets' }),
  ...
  describe('App List Query', () => {
    ...
    it('should remove legacy tagIDs from URL while preserving other filters', async () => {
      ...
      await waitFor(() => {
        expect(mockReplace).toHaveBeenCalledWith('/apps?category=workflow&keywords=sales', {
          scroll: false,
        })
      })
    })

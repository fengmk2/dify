...
- Browser failures keep screenshots under `web/.vitest/attachments/` and Playwright traces under `web/.vitest-browser/`. CI uploads these directories only when failure artifacts exist; Browser Mode does not own coverage or report merging.

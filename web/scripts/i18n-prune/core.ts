...
function getTranslationFunctionInfoFromType(
  ...
  if (isLikelyTranslationFunctionType(typeNode)) return { namespaces: [], keyPrefix: undefined }
...
function addTranslationFunction(
  scope: Scope,
  name: string,
  info: TranslationFunctionInfo = { namespaces: [], keyPrefix: undefined },
) {
...
function evaluateStaticExpression(expression: ts.Expression, scopes: Scope[]): StaticValue {
  ...
  if (ts.isElementAccessExpression(unwrapped) && unwrapped.argumentExpression) {
    ...
    if (container.kind === 'array') {
      ...
      if (argument.values.length !== 1) return unknownStaticValue()

      const key = argument.values[0]!
      const index = Number(key)
      return Number.isInteger(index)
        ? (container.elements[index] ?? unknownStaticValue())
        : unknownStaticValue()
    }
  }
...
function collectStringValues(value: StaticValue): string[] {
  ...
  if (value.kind === 'object')
    return uniqueSorted(
      Array.from(value.properties.values()).flatMap((property) => collectStringValues(property)),
    )
  ...
  function handleVariableDeclaration(node: ts.VariableDeclaration) {
    ...
    const translationInfo =
      calleeName === 'useTranslation'
        ...
        : {
            namespaces: evaluateNamespaceExpression(
              callExpression.arguments[1] as ts.Expression | undefined,
              scopes,
              catalog,
            ),
            keyPrefix: undefined,

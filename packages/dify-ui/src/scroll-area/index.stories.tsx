  ...
  return (
    <section className={cn(pageClassName, className)}>
      <div className="space-y-1">
        ...
        <p className="max-w-[72ch] text-pretty system-sm-regular text-text-secondary">
          {description}
        </p>
      </div>
            ...
            <ScrollAreaContent className="min-h-full min-w-max p-4 pb-6">
              <div className="grid grid-cols-[repeat(18,6.25rem)] gap-3">
                {gridCells.slice(0, 18).map((cell) => (
                  <div
                    key={cell}
                    className="flex h-24 items-center justify-center rounded-xl border border-divider-subtle bg-components-panel-bg-alt tabular-nums system-md-semibold text-text-secondary"
                  >
                    {cell}
                  </div>
                ))}
            ...
            <ScrollAreaContent className="pt-3 pr-6 pb-6 pl-3">
              <div className="grid grid-cols-[repeat(10,6.25rem)] grid-rows-[repeat(10,6.25rem)] gap-3">
                {gridCells.map((cell) => (
                  <div
                    key={cell}
                    className="flex items-center justify-center rounded-lg border border-divider-subtle bg-components-panel-bg-alt tabular-nums system-md-semibold text-text-secondary"
                  >
                    {cell}
                  </div>
                ))}
